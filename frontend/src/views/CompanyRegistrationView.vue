  ...
  <MainLayout>
    <div class="container">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <BreadcrumbComponent pageName="Osaühingu registreerimine" @home-click="goHome" />
        ...
        <div class="shadow-sm rounded bg-white p-4 mb-4">
          ...
          <div class="row mb-3">
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control" id="companyName" v-model="company.name" :class="{ 'is-invalid': errors.name }" placeholder="3-100 tähemärki" required>
              <div v-if="errors.name" class="invalid-feedback">{{ errors.name }}</div>
            </div>
            <div class="col-md-6 mb-3">
              <label for="regCode" class="form-label">Registrikood<span class="text-danger">*</span></label>
              <input type="text" class="form-control" id="regCode" v-model="company.regCode" :class="{ 'is-invalid': errors.regCode }" placeholder="7-kohaline number" required>
          ...
          <div class="row mb-3">
            <div class="col-md-6 mb-3">
              ...
              <input type="date" class="form-control" id="foundingDate" v-model="company.foundingDate" :class="{ 'is-invalid': errors.foundingDate }" :max="today" required>
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="number" class="form-control" id="capital" v-model.number="company.capital" :class="{ 'is-invalid': errors.capital }" min="2500" placeholder="Vähemalt 2500€" required>
          ...
          <div v-if="company.shareholders.length > 0" class="alert alert-secondary mb-4">
            ...
            <div class="progress" style="height: 6px;">
              <div class="progress-bar" :class="calculatedCapital === company.capital ? 'bg-success' : ''" role="progressbar" :style="{ width: capitalPercentage + '%' }" aria-valuemin="0" aria-valuemax="100"></div>
            </div>
          </div>
          ...
          <div v-for="(shareholder, index) in company.shareholders" :key="index" class="mb-4 p-3 border rounded shadow-sm">
            <div class="d-flex justify-content-between align-items-center mb-3">
              ...
              <button type="button" class="btn btn-danger" @click="removeShareholder(index)" :disabled="company.shareholders.length === 1">
            ...
            <div class="mb-3">
              ...
              <div class="btn-group" role="group">
                <input type="radio" class="btn-check" :name="'shareholderType' + index" :id="'individual' + index" value="individual" v-model="shareholder.type" />
                <label class="btn btn-outline-primary" :for="'individual' + index">Füüsiline isik</label>
                <input type="radio" class="btn-check" :name="'shareholderType' + index" :id="'legal' + index" value="legal" v-model="shareholder.type" />
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" v-model="shareholder.searchQuery" @input="searchPerson(index)" placeholder="Otsi..." />
              ...
              <ul v-else-if="shareholder.searchResults && shareholder.searchResults.length" class="list-group mt-2">
                <li v-for="(result, rIndex) in shareholder.searchResults" :key="rIndex" class="list-group-item list-group-item-action" @click="selectPerson(index, result)">
                  {{ result.display }}
                </li>
              </ul>
            ...
            <div v-if="shareholder.type === 'individual'" class="mb-3">
              <div class="row">
                <div class="col-md-4 mb-3">
                  ...
                  <input type="text" class="form-control" :id="'firstName' + index" v-model="shareholder.firstName" required />
                </div>
                <div class="col-md-4 mb-3">
                  <label :for="'lastName' + index" class="form-label">Perenimi<span class="text-danger">*</span></label>
                  <input type="text" class="form-control" :id="'lastName' + index" v-model="shareholder.lastName" required />
                </div>
                <div class="col-md-4 mb-3">
                  <label :for="'idCode' + index" class="form-label">Isikukood<span class="text-danger">*</span></label>
                  <input type="text" class="form-control" :id="'idCode' + index" v-model="shareholder.idCode" required />
                </div>
              </div>
            </div>
            <!-- Legal person fields -->
            <div v-if="shareholder.type === 'legal'" class="mb-3">
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="text" class="form-control" :id="'legalName' + index" v-model="shareholder.legalName" required />
                </div>
                <div class="col-md-6 mb-3">
                  <label :for="'legalCode' + index" class="form-label">Registrikood<span class="text-danger">*</span></label>
                  <input type="text" class="form-control" :id="'legalCode' + index" v-model="shareholder.legalCode" required />
                </div>
              </div>
            </div>
            <!-- Share -->
            <div class="mb-3">
              ...
              <input type="number" class="form-control" :id="'share' + index" v-model.number="shareholder.share" :class="{ 'is-invalid': errors['share'+index] }" min="1" placeholder="Vähemalt 1€" required />
          ...
          <div class="text-end mt-4">
            <button type="button" class="btn btn-secondary me-2" @click="goHome">Tühista</button>
  ...
  methods: {
      ...
      try {
        ...
        this.company = {
          ...
          shareholders: (data.shareholders || []).map(s => ({
            id: s.person.id, // use person id if available
            type: s.person.type,
            share: Number(s.share),
            ...(s.person.type === 'individual'
              ? {
                  firstName: s.person.first_name,
                  lastName: s.person.last_name,
                  idCode: s.person.id_code,
                  searchQuery: `${s.person.first_name} ${s.person.last_name}`,
                  legalName: '',
                  legalCode: ''
                }
              : {
                  legalName: s.person.legal_name,
                  legalCode: s.person.reg_code,
                  searchQuery: s.person.legal_name,
                  firstName: '',
                  lastName: '',
                  idCode: ''
                }),
    ...
    async submitForm() {
      ...
      const registrationData = {
        ...
        shareholders: this.company.shareholders.map(sh => {
          if (sh.type === 'individual') {
            return {
              id: sh.id || null,  // if existing person was selected include their id
              type: sh.type,
              first_name: sh.firstName,
              last_name: sh.lastName,
              id_code: sh.idCode,
              share: String(sh.share)
            }
          } else {
            return {
              id: sh.id || null,
              type: sh.type,
              legal_name: sh.legalName,
              reg_code: sh.legalCode,
              share: String(sh.share)
            }
          }
        })
      ...
      try {
        const response = await axios.post(`${apiBaseUrl}/companies/registration`, registrationData)
        alert('Osaühing edukalt registreeritud!')
